...
var workbook = XLSX.readFile('./assets/geocoord.xlsx');
workbook.SheetNames.forEach(name => {
    weilanSource = XLSX.utils.sheet_to_json(workbook.Sheets[name]);
    let lat = weilanSource.map(item => item['经度']);
    let lng = weilanSource.map(item => item['纬度']);
    areaLatlng[0] = Math.max.apply(null,lat);
    areaLatlng[1] = Math.max.apply(null,lng);
    areaLatlng[2] = Math.min.apply(null,lat);
    areaLatlng[3] = Math.min.apply(null,lng);
});
...
function getAroundGroup(x,y,circle){
    ...
    let x2 = x + circle <= mixNum ? (x + circle) : mixNum;
    let y1 = y - circle > 0 ? (y - circle) : 0;
    let y2 = y + circle <= mixNum ? (y + circle) : mixNum;
    weilanData.forEach((item, i) => {
        if(item.checked == 1) return;
        let name = item.name.split('-');
...
function writeXLXSFile() {
    ...
    XLSX.writeFile(wb, "./assets/geocoordParse.xlsx");
